...
bids_sub = ['sub-ccepAgeUMCU' input('Patient number: sub-ccepAgeUMCU(XXXX): ', 's')];
bids_ses = ['ses-' input('Session number: ses-(X): ', 's')];
bids_task = 'task-SPESclin';

% choose between available runs
files = dir(fullfile(myDataPath.input, bids_sub, bids_ses, 'ieeg', [bids_sub '_' bids_ses '_' bids_task '_*'  '_events.tsv']));
names = {files.name};
